    ...
    if (adminRequest) {
      ...
      if (customerToken && isNumeric(cartId)) {
        return this.restClient.put(
          "/carts/mine/coupons/" + coupon,
          null,
          customerToken
        );
      } else {
        return this.restClient.put(
          "/guest-carts/" + cartId + "/coupons/" + coupon
        );
      }
    }
  }
  ...
  async delete(
    ...
    if (adminRequest) {
      return this.restClient.delete(
        "/carts/" + cartId + "/items/" + cartItem.item_id
      );
    } else {
      if (customerToken && isNumeric(cartId)) {
        return this.restClient.delete(
          "/carts/mine/items/" + cartItem.item_id,
          customerToken
        );
      } else {
        return this.restClient.delete(
          "/guest-carts/" + cartId + "/items/" + cartItem.item_id
        );
      }
    }
  }
